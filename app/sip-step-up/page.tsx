  ...
  return (
    <CalculatorShell title="Step-Up SIP Calculator">
      <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] mb-6">
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <label className="flex flex-col gap-2 rounded-3xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950">
              <span className="text-sm text-zinc-500">Annual Step-Up</span>
              <input
                type="number"
                value={stepUp}
                onChange={(e) => setStepUp(+e.target.value)}
                className="w-full rounded-3xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none transition focus:border-cyan-500 focus:ring-2 focus:ring-cyan-500/20 dark:bg-zinc-900 dark:text-white dark:border-zinc-700"
                placeholder="Annual Step-Up %"
              />
            </label>
            <label className="flex flex-col gap-2 rounded-3xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div className="grid gap-4">
          ...
          <div className="rounded-3xl border border-zinc-200 bg-zinc-50 p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <p className="mt-3 text-2xl font-semibold text-zinc-900 dark:text-white">{formatCurrency(wealthGained)}</p>
          </div>
      ...
      <div className="rounded-3xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950">
        <p className="text-sm text-zinc-500">Step-up SIP growth</p>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {chartData.map((item) => (
          <div key={item.year} className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-950">
            <p className="text-sm text-zinc-500">Year {item.year}</p>
            <p className="mt-2 text-lg font-semibold">{formatCurrency(item.balance)}</p>
            <p className="text-xs text-zinc-500">Invested {formatCurrency(item.invested)}</p>
          </div>
        ))}
      </div>
    </CalculatorShell>
